...
psql -U %pguser% -d %pgdb% -c ^
    "create table if not exists water.wikidata_waterways_russia (waterway text, labelru text, labelen text, sourcecoords text, mouthcoords text, mouthqid text, length numeric, gvr text, osm_id int8, tributaries text); truncate water.wikidata_waterways_russia;"
